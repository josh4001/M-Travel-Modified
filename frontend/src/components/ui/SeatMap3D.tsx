...
export function SeatMap3D() {
  ...
  return (
    <div className="card-luxe bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          ...
          <h3 className="font-display text-xl font-bold text-slate-900">Nairobi → Mombasa Express</h3>
        </div>

        <div className="flex items-center gap-4 text-xs font-semibold text-slate-700">
          <span className="flex items-center gap-1.5"><span className="h-3 w-3 rounded bg-slate-800 border border-slate-700" /> Available</span>
          <span className="flex items-center gap-1.5"><span className="h-3 w-3 rounded bg-amber-500" /> Selected</span>
          <span className="flex items-center gap-1.5"><span className="h-3 w-3 rounded bg-red-200 border border-red-300" /> Booked</span>
        </div>
      </div>
      ...
      <div className="perspective-1000 my-8 py-4">
        <motion.div
          ...
          className="mx-auto max-w-md rounded-2xl border border-slate-800 bg-slate-950 p-6 shadow-xl preserve-3d"
        >
          <div className="mb-6 flex justify-between items-center border-b border-slate-800 pb-3 text-xs text-slate-400 font-bold">
            <span>DRIVER CAB</span>
            <span className="rounded bg-teal/20 px-2 py-0.5 text-teal font-mono">FRONT</span>
          </div>
              ...
              return (
                <motion.button
                  ...
                  className={`relative flex flex-col items-center justify-center rounded-xl p-3 text-xs transition-all duration-200 ${
                    isBooked
                      ? 'bg-slate-900/60 border border-red-500/20 text-red-400/50 cursor-not-allowed'
                      : isSelected
                      ? 'bg-amber-500 text-slate-950 font-bold border-2 border-white shadow-md transform -translate-z-4'
                      : 'bg-slate-900 border border-slate-700 text-slate-200 hover:border-amber-400 hover:bg-slate-800'
                  }`}
                >
                  <Armchair className="h-5 w-5 mb-1" />
                  <span className="font-bold">{seat.number}</span>
                  <span className="mt-0.5 text-[10px] opacity-75 font-mono">{seat.id}</span>
                </motion.button>
              );
            })}
          </div>
      ...
      <div className="flex items-center justify-between border-t border-slate-200 pt-4 text-sm">
        <div>
          ...
          <span className="font-display font-bold text-amber-700">{selectedSeat || 'None'}</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="font-mono text-base font-bold text-amber-700">KES 1,800</span>
          <button className="btn-primary !py-2 !px-4 text-xs font-bold shadow-sm">
