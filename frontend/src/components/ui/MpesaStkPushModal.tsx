...
export const MpesaStkPushModal: React.FC<MpesaStkPushModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-md font-display">
      <div className="relative w-full max-w-md rounded-3xl border border-emerald-500/40 bg-gradient-to-b from-ink-100 via-ink to-mtravel-obsidian p-6 shadow-3d-glow text-bone">
        {/* CLOSE BUTTON */}
        <button
          onClick={onClose}
          className="absolute right-4 top-4 rounded-full bg-white/10 p-2 text-bone/60 hover:bg-white/20 hover:text-bone transition"
        >
          <X className="h-4 w-4" />
        </button>
        ...
        <div className="flex items-center gap-3 border-b border-white/10 pb-4">
          ...
          <div>
            ...
            <h3 className="font-serif text-lg font-bold text-bone">Real-time Payment Gateway</h3>
          </div>
        </div>
          ...
          <form onSubmit={handleSendStk} className="mt-5 space-y-4">
            <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-xs space-y-2">
              <div className="flex justify-between text-bone/70">
                ...
                <span className="font-semibold text-bone">{vehicleName}</span>
              </div>
              <div className="flex justify-between text-bone/70">
                <span>Booking Reference:</span>
                <span className="font-mono text-mtravel-lightGold font-bold">{bookingRef}</span>
              </div>
              <div className="flex justify-between border-t border-white/10 pt-2 font-bold text-sm text-emerald-400">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="input-field text-sm !py-3 font-mono border-emerald-500/40 focus:border-emerald-400"
                value={phone}
                onChange={(e) => setPhone(e.target.value)}
              />
              <p className="text-[11px] text-bone/60 flex items-center gap-1">
        ...
        {step === 'ENTER_PIN' && (
          <div className="mt-5 space-y-4 text-center">
            ...
            <div className="mx-auto w-full max-w-xs rounded-2xl border-2 border-white/30 bg-black/90 p-4 shadow-2xl space-y-3 text-left font-mono">
              <div className="flex items-center justify-between border-b border-white/20 pb-2 text-[10px] text-bone/60">
                <span>SIM 1 - M-PESA</span>
                <span>Safaricom</span>
              </div>
              <p className="text-xs text-bone font-bold leading-relaxed">
              ...
              <div className="flex items-center justify-center gap-3 py-2 bg-white/10 rounded-lg text-lg tracking-widest font-bold text-emerald-400">
                {pin ? '•'.repeat(pin.length) : <span className="text-xs text-bone/40">Enter 4-digit PIN</span>}
              </div>
        ...
        {step === 'SUCCESS' && (
          <div className="mt-5 text-center space-y-4 py-4">
            ...
            <div>
              <h4 className="font-serif text-xl font-bold text-bone">Payment Received via M-PESA!</h4>
              <p className="text-xs text-bone/70 mt-1">Transaction confirmed by Safaricom Daraja</p>
            </div>

            <div className="rounded-2xl border border-emerald-500/30 bg-black/40 p-4 font-mono text-xs text-left space-y-2">
              <div className="flex justify-between">
                <span className="text-bone/60">Receipt Code:</span>
                <span className="font-bold text-emerald-400">{receipt}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-bone/60">Amount Paid:</span>
                <span className="font-bold text-bone">KES {amount.toLocaleString()}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-bone/60">Booking Ref:</span>
                <span className="font-bold text-mtravel-lightGold">{bookingRef}</span>
              </div>
              <div className="flex justify-between">
            ...
            <p className="text-xs text-mtravel-lightGold font-mono">
              Redirecting to Live Uber/OpenCV Car View...
            </p>
          </div>
        )}

        <div className="mt-5 border-t border-white/10 pt-3 flex items-center justify-between text-[10px] text-bone/50">
