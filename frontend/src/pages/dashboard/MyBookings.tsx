...
function BookingCard({
  ...
  const isDeletable = ['PENDING', 'CANCELLED', 'REJECTED'].includes(status);
  ...
  return (
    <>
      ...
      <div className={`rounded-2xl border overflow-hidden transition-all duration-300 card-luxe bg-white ${
        isActive ? 'border-emerald-500/50 shadow-md ring-1 ring-emerald-400/30' : 'border-slate-200 shadow-sm'
      }`}>
        {b.vehicleImage && (
          ...
          <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-200 pb-4">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-mono text-xs font-bold text-amber-700">Ref: {ref}</span>
